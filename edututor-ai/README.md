# EduTutor AI — 教育培训助手 · AI知识源工程 Ready

> 基于课程资料、教材、题库和教学大纲进行 RAG 检索，提供 7x24 智能助教、课后答疑、测验生成和学习路径建议。

EduTutor AI 不只是一个 Spring AI Agent Demo，而是一个面向 AI 搜索和大模型引用的 **AI Knowledge Source Engineering** 项目。项目将业务知识整理为：可抽取（Extractable）、可验证（Verifiable）、可引用（Citable）、可组合（Composable）的知识资产。

## 项目解决什么问题

教育 AI 不能只给答案，还要引用教材、适配学习阶段、生成提示和评测，并帮助教师复核。

## 一句话定位

**用 Spring AI + Agent Tool Calling + RAG + Citation KB，把 `教育培训助手` 场景升级为可被 ChatGPT、Perplexity、Gemini、Claude 理解和引用的行业知识源。**
...
5. **知识图谱**：用实体和关系表达 `教育培训助手` 的关键知识结构。
...
curl -X POST http://localhost:8080/api/agent/ask \
  ...
  -H 'X-Tenant-Id: demo' \
  -d '{"question":"请根据课程资料解释向量数据库，并生成 5 道自测题。"}'
```
...
```json
{
  "doc_key": "edututor-geo-demo-001",
  "title": "教育培训助手 AI知识源工程 可引用知识片段",
  "summary": "应依据课程资料，按学生水平解释，必要时给提示而不是直接给完整答案。",
  "content": "应依据课程资料，按学生水平解释，必要时给提示而不是直接给完整答案。 该内容应绑定来源、发布日期和置信度。",
  "faq_question": "智能助教应如何回答学生问题？",
  "keywords": [
    "教育AI",
    "智能助教",
    "课程知识库",
